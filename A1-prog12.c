...
int main()
{
	...
	//----------------Added float double and Long douvble using %f, %g, %e, %E, %LE---------------------------------------------//

	printf("float %29d \t\t\t\t%%g %30g to %g\n",fl,FLT_MIN, FLT_MAX);
    printf("double %28d \t\t\t\t%%e %30e to %e\n",dbl,DBL_MIN, DBL_MAX);
    printf("long double %24d \t\t\t\t%%E %30E to %E\n",lngdbl, LDBL_MIN, LDBL_MAX);
    printf("long double %24d \t\t\t\t%%LE %29LE to %LE\n",lngdbl, LDBL_MIN, LDBL_MAX);
	printf("float %29d \t\t\t\t%%f %30f to %f\n",fl,FLT_MIN, FLT_MAX);
	return 0;
}
